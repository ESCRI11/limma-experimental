...
{
  # saveRDS(M, "~/libs/test_ewas/data/M.rds")
  # saveRDS(design, "~/libs/test_ewas/data/design.rds")
  # saveRDS(ndups, "~/libs/test_ewas/data/ndups.rds")
  # saveRDS(spacing, "~/libs/test_ewas/data/spacing.rds")
  # saveRDS(weights, "~/libs/test_ewas/data/weights.rds")
  ...
  FUN <- function(i){
    stdev.unscaled <- beta <- matrix(NA,length(i),nbeta,byrow=TRUE)
    sigma <- rep_len(NA_real_,ngenes)
    df.residual <- rep_len(0,ngenes)
    results <- lapply(i, function(i){
      y <- as.vector(M[i,])
      obs <- is.finite(y)
      X <- design[obs,,drop=FALSE]
      y <- y[obs]
      ...
      if(length(y) > nbeta) {
        out <- MASS::rlm(x=X,y=y,weights=w)
        beta <- coef(out)
        stdev.unscaled <- sqrt(diag(chol2inv(out$qr$qr)))
        df.residual <- length(y) - out$rank
        if(df.residual > 0) sigma <- out$s
        return(list(beta, stdev.unscaled, df.residual, sigma))
      }
    })
    for (x in 1:length(results)){
      beta[x,] <- results[[x]][1][[1]]
      stdev.unscaled[x,] <- results[[x]][2][[1]]
      df.residual[x] <- results[[x]][3][[1]]
      sigma[x] <- results[[x]][4][[1]]
    }
    return(list(beta, stdev.unscaled, df.residual, sigma))
  }
  step <- floor(ngenes / n_workers)
  indexes <- lapply(1:n_workers, function(x){
    if(x == 1){
      1:step
    } else if (x == n_workers){
      ((step*(x-1))+1):ngenes
  ...
  for (x in 1:length(results)){
    beta[indexes[[x]],] <- results[[x]][1][[1]]
    stdev.unscaled[indexes[[x]],] <- results[[x]][2][[1]]
    df.residual[indexes[[x]]] <- results[[x]][3][[1]]
    sigma[indexes[[x]]] <- results[[x]][4][[1]]
  }
